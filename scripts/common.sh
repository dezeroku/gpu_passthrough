...
template_xml() {
	...
	docker run --rm -v "${SCRIPTS_DIR}/../vm_config.yaml:/config/vm_config.yaml" --env-file "${env_file}" -i "${DOCKER_GOMPLATE_IMAGE}" -c 'config=/config/vm_config.yaml' <"${xml_file}" >"${tmp_file}"
